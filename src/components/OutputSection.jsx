...
function OutputSection({ summary, results }) {
  ...
  return (
    <section className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md transition-colors duration-200">
      ...
      {/* Overall Summary */}
      <div
        className={`p-4 rounded-lg border mb-6 ${getSentimentBg(summary)}`}
      >
        <h3 className="font-semibold text-gray-900 dark:text-white mb-2">
          Overall Sentiment
        </h3>
        <span
          className={`text-lg font-bold ${getSentimentColor(summary)}`}
        >
          {summary}
        </span>
      </div>
          ...
          <tbody>
            {results.map((result, index) => (
              ...
              >
                ...
                <td className="border border-gray-200 dark:border-gray-600 px-4 py-3 text-gray-600 dark:text-gray-300 italic">
                  {result.explanation || "—"}
                </td>
              </tr>
            ))}
